# Heltec V3 WiFi TCP Bridge for MeshCore

Build system for MeshCore firmware on Heltec V3 that bridges LoRa mesh packets over WiFi TCP.
...
- **Device**: Heltec V3 microcontroller + LoRa radio running MeshCore firmware
...
- **Display**: Built-in OLED for status & UI

## Features

- ✅ One-command build & upload to Heltec V3
...
- ✅ OLED display shows WiFi status & IP address

## Prerequisites

- Heltec V3 microcontroller with LoRa radio
...
```bash
git clone https://github.com/zm0ra/meshcore-heltec-v3-serial2tcp.git
cd meshcore-heltec-v3-serial2tcp
```
...
[CONSOLE] TCP console started on <ip>:5001 (repeater only)
```
...
- **OLED display** shows WiFi status, IP address, and packet stats
...
| | LORA_TX_POWER | 22 | TX power (dBm, max 27 for Heltec V3) |
| | MAX_CONTACTS | 350 | Max stored contacts |
| | MAX_GROUP_CHANNELS | 40 | Max group channels |
| **Display** | DISPLAY_CLASS | SSD1306Display | I2C OLED driver |
| **Identity** | ADVERT_NAME | Heltec V3 WiFi | Your node name on the mesh |
...
- Ensure network is 2.4GHz (Heltec V3 doesn't support 5GHz)
...
- Install Heltec V3 USB drivers (usually automatic on modern systems)
...
```
meshcore-heltec-v3-serial2tcp/
├── build.sh                            # Main build script
├── config.env.example                  # Example configuration
├── README.md                           # This file
├── mesh_client.py                      # Python TCP client
├── patches/
│   ├── 01-mymesh-header.patch         # MyMesh.h TCP bridge additions
│   └── 02-mymesh-implementation.patch # MyMesh.cpp implementation
└── build/
...
- **LoRa Radio**: SX1262 (UHF)
- **Display**: 0.96" OLED I2C
- **Battery**: 3.7V Li-Po (1200 mAh)
- **Antenna**: Internal + external connector
- **USB**: USB-C for upload & serial
...
- USB powered: ~200mA idle, 600mA TX
- Battery mode: Use deep sleep for extended operation
- Efficient WiFi scanning reduces power draw
